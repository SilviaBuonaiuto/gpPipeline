...
ggplot(df, aes(x,y)) + geom_line() + geom_ribbon(aes(ymin=0, ymax=y, fill=quant)) + xlab("Number of variants per gene") + xlab("Number of variants per gene") + ylab("") + theme_bw() + ggtitle("Variants per gene density")
...
#number$impact= factor(number$impact,levels=c('HIGH','MODERATE'))
nbcolors=length(levels(as.factor(myd$Most_severe))) 
...
geneTranscripts %>% gather ('Feature_type','n', c(genes, transcripts)) %>% ggplot(aes(as.factor(sample) , n , color=Feature_type) ) + geom_point() + ggtitle(paste (code, '- Genes and transcripts', sep=' ')) + theme_bw() + scale_color_nejm() + xlab("")
...
############ 
print('plicadsumgene')
myd %>% ggplot(aes(pLIscore, CADD_RAW, color=as.factor(sumGene))) + geom_point() + ggtitle(paste(code, '- pLI CADD sumGene', sep=' ')) + theme_bw() + scale_color_futurama() + labs(color = "Gene lists")
