...
def main():
	...
	c.execute('CREATE TABLE freqTable1Ctr (Uploaded_variation text,Location text,Allele text,Gene text,Feature text,Feature_type text,Consequence text,cDNA_position integer,CDS_position integer,Protein_position integer,Amino_acids text,Codons text,Existing_variation text,IMPACT text,SYMBOL text,STRAND text,SIFT real,PolyPhen real,EXON integer,AF real,AFR_AF real,AMR_AF real,EAS_AF real,EUR_AF real,SAS_AF real,gnomAD_AF real,gnomAD_AFR_AF real,gnomAD_AMR_AF real,gnomAD_ASJ_AF real,gnomAD_EAS_AF real,gnomAD_FIN_AF real,gnomAD_NFE_AF real,gnomAD_OTH_AF real,gnomAD_SAS_AF real,CADD_RAW real, CADD_PHRED real, pLIscore real, EmbryoDev real, DDD real, Lethal real, Essential real, Misc real, DDR real, Candidate real, index_x text, rare01 text);')

	c.execute("INSERT INTO freqTable1Ctr SELECT *, CASE WHEN AFR_AF =0 AND AMR_AF =0 AND EAS_AF =0 AND EUR_AF =0 AND SAS_AF =0 AND gnomAD_AFR_AF =0 AND gnomAD_AMR_AF =0 AND gnomAD_ASJ_AF =0 AND gnomAD_EAS_AF =0 AND gnomAD_FIN_AF =0 AND gnomAD_NFE_AF =0 AND gnomAD_OTH_AF =0 AND gnomAD_SAS_AF =0 THEN 'NOB' WHEN AFR_AF <=? AND AMR_AF <=? AND EAS_AF <=? AND EUR_AF <=? AND SAS_AF <=? AND gnomAD_AFR_AF <=? AND gnomAD_AMR_AF <=? AND gnomAD_ASJ_AF <=? AND gnomAD_EAS_AF <=? AND gnomAD_FIN_AF <=? AND gnomAD_NFE_AF <=? AND gnomAD_OTH_AF <=? AND gnomAD_SAS_AF <=? THEN 'true' ELSE 'false' END FROM indexTable;" ,(thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,))

	conn.commit()

	thr2 = args.f
	c.execute("DROP TABLE IF EXISTS freqTableCtr;")
	c.execute('CREATE TABLE freqTableCtr (Uploaded_variation text,Location text,Allele text,Gene text,Feature text,Feature_type text,Consequence text,cDNA_position integer,CDS_position integer,Protein_position integer,Amino_acids text,Codons text,Existing_variation text,IMPACT text,SYMBOL text,STRAND text,SIFT real,PolyPhen real,EXON integer,AF real,AFR_AF real,AMR_AF real,EAS_AF real,EUR_AF real,SAS_AF real,gnomAD_AF real,gnomAD_AFR_AF real,gnomAD_AMR_AF real,gnomAD_ASJ_AF real,gnomAD_EAS_AF real,gnomAD_FIN_AF real,gnomAD_NFE_AF real,gnomAD_OTH_AF real,gnomAD_SAS_AF real, CADD_RAW real, CADD_PHRED real, pLIscore real, EmbryoDev real, DDD real, Lethal real, Essential real, Misc real, DDR real, Candidate real, index_x text, rare01 real, rare05 text);')

	c.execute("INSERT INTO freqTableCtr SELECT *, CASE WHEN AFR_AF =0 AND AMR_AF =0 AND EAS_AF =0 AND EUR_AF =0 AND SAS_AF =0 AND gnomAD_AFR_AF =0 AND gnomAD_AMR_AF =0 AND gnomAD_ASJ_AF =0 AND gnomAD_EAS_AF =0 AND gnomAD_FIN_AF =0 AND gnomAD_NFE_AF =0 AND gnomAD_OTH_AF =0 AND gnomAD_SAS_AF =0 THEN 'NOB' WHEN AFR_AF <=? AND AMR_AF <=? AND EAS_AF <=? AND EUR_AF <=? AND SAS_AF <=? AND gnomAD_AFR_AF <=? AND gnomAD_AMR_AF <=? AND gnomAD_ASJ_AF <=? AND gnomAD_EAS_AF <=? AND gnomAD_FIN_AF <=? AND gnomAD_NFE_AF <=? AND gnomAD_OTH_AF <=? AND gnomAD_SAS_AF <=? THEN 'true' ELSE 'false' END FROM freqTable1Ctr;" ,(thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,))

	conn.commit()
	###### create sumGene column in control samples df
	c.execute("DROP TABLE IF EXISTS sumgeneTabCtr;")
	c.execute('CREATE TABLE sumgeneTabCtr (Uploaded_variation text,Location text,Allele text,Gene text,Feature text,Feature_type text,Consequence text,cDNA_position integer,CDS_position integer,Protein_position integer,Amino_acids text,Codons text,Existing_variation text,IMPACT text,SYMBOL text,STRAND text,SIFT real,PolyPhen real,EXON integer,CADD_RAW real, CADD_PHRED real, pLIscore real, EmbryoDev real, DDD real, Lethal real, Essential real, Misc real, DDR real, Candidate real, index_x text, rare01 text, rare05 text, sumGene real);')

	c.execute("INSERT INTO sumgeneTabCtr SELECT Uploaded_variation,Location,Allele,Gene,Feature,Feature_type,Consequence,cDNA_position,CDS_position,Protein_position,Amino_acids,Codons,Existing_variation,IMPACT,SYMBOL,STRAND,SIFT,PolyPhen,EXON,CADD_RAW, CADD_PHRED, pLIscore, EmbryoDev, DDD, Lethal, Essential, Misc, DDR, Candidate, index_x, rare01,rare05, (EmbryoDev + DDD + Lethal + Essential + Misc + DDR + Candidate ) FROM freqTableCtr;")

	conn.commit()

	###### create CADD percentile column in control samples df
	c.execute("DROP TABLE IF EXISTS finalTableCtr;")
	c.execute("CREATE TABLE finalTableCtr (Uploaded_variation text,Location text,Allele text,Gene text,Feature text,Feature_type text,Consequence text,cDNA_position integer,CDS_position integer,Protein_position integer,Amino_acids text,Codons text,Existing_variation text,IMPACT text,SYMBOL text,STRAND text,SIFT real,PolyPhen real,EXON integer,CADD_RAW real, CADD_PHRED real, pLIscore real, EmbryoDev real, DDD real, Lethal real, Essential real, Misc real, DDR real, Candidate real, index_x text, rare01 text, rare05 text, sumGene real, caddPercent real);")
	...
	c.execute("INSERT INTO freqTable1 SELECT *, CASE WHEN AFR_AF =0 AND AMR_AF =0 AND EAS_AF =0 AND EUR_AF =0 AND SAS_AF =0 AND gnomAD_AFR_AF =0 AND gnomAD_AMR_AF =0 AND gnomAD_ASJ_AF =0 AND gnomAD_EAS_AF =0 AND gnomAD_FIN_AF =0 AND gnomAD_NFE_AF =0 AND gnomAD_OTH_AF =0 AND gnomAD_SAS_AF =0 THEN 'NOB' WHEN AFR_AF <=? AND AMR_AF <=? AND EAS_AF <=? AND EUR_AF <=? AND SAS_AF <=? AND gnomAD_AFR_AF <=? AND gnomAD_AMR_AF <=? AND gnomAD_ASJ_AF <=? AND gnomAD_EAS_AF <=? AND gnomAD_FIN_AF <=? AND gnomAD_NFE_AF <=? AND gnomAD_OTH_AF <=? AND gnomAD_SAS_AF <=? THEN 'true' ELSE 'false' END FROM indexTable;" ,(thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,thr,))
	...
	c.execute("INSERT INTO freqTable SELECT *, CASE WHEN AFR_AF =0 AND AMR_AF =0 AND EAS_AF =0 AND EUR_AF =0 AND SAS_AF =0 AND gnomAD_AFR_AF =0 AND gnomAD_AMR_AF =0 AND gnomAD_ASJ_AF =0 AND gnomAD_EAS_AF =0 AND gnomAD_FIN_AF =0 AND gnomAD_NFE_AF =0 AND gnomAD_OTH_AF =0 AND gnomAD_SAS_AF =0 THEN 'NOB' WHEN AFR_AF <=? AND AMR_AF <=? AND EAS_AF <=? AND EUR_AF <=? AND SAS_AF <=? AND gnomAD_AFR_AF <=? AND gnomAD_AMR_AF <=? AND gnomAD_ASJ_AF <=? AND gnomAD_EAS_AF <=? AND gnomAD_FIN_AF <=? AND gnomAD_NFE_AF <=? AND gnomAD_OTH_AF <=? AND gnomAD_SAS_AF <=? THEN 'true' ELSE 'false' END FROM freqTable1;" ,(thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,thr2,))
